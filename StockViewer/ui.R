...
navbarPage("Stock Viewer", theme = shinytheme("flatly"), 
  ...
  tabPanel("Main",
      sidebarPanel(tags$style(type="text/css",
                ...
                splitLayout(downloadButton("stock_price", "OHLC Prices", style = "Width:200px;display:block;margin:0 auto;margin-top:5px;margin-bot:5px"), 
                            downloadButton("splits", "Splits", style = "Width:200px;display:block;margin:0 auto;margin-top:5px;margin-bot:5px")),
  ...
  tabPanel("About", 
           mainPanel(
             ...
             helpText("Stock Viewer is meant to be a fast and easy platform for both students and working professionals to download stock market data. All information shown can be downloaded as a CSV file, which can easily be opened in Excel or any other platform."),
             ...
             actionButton(inputId = "", label = "" , style = "background:#ffffff;color:#2C3E50;border-color:#2C3E50" , icon = icon("linkedin"), 
                          onclick ="window.open('https://www.linkedin.com/in/mart%C3%ADn-rodr%C3%ADguez-frumento-19773b149/', '_blank')"),
             actionButton(inputId = "", label = "" , style = "background:#ffffff;color:#2C3E50;border-color:#2C3E50" , icon = icon("github"), 
